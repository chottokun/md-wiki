        ...
        try:
            ...
            if data is None:
                result.errors.append(
                    f"❌ {f.relative_to(wiki_dir)}: Missing or invalid YAML frontmatter"
                )
                continue
            ...
            if not doc_type:
                result.errors.append(
                    f"❌ {f.relative_to(wiki_dir)}: Required 'type' field is missing or empty"
                )
            else:
            ...
            if "description" not in data or not data["description"]:
                ...
                result.warnings.append(
                    f"⚠️  {f.relative_to(wiki_dir)}: Missing recommended 'description' field"
                )
            if "timestamp" not in data or not data["timestamp"]:
                result.missing_timestamp_count += 1
                result.warnings.append(
                    f"⚠️  {f.relative_to(wiki_dir)}: Missing recommended 'timestamp' field"
                )
        except Exception as e:
            result.errors.append(
                f"❌ {f.relative_to(wiki_dir)}: Failed to read/parse: {e}"
            )
    ...
    for idx_file in index_files:
        try:
            ...
            if content.startswith("---"):
                result.errors.append(
                    f"❌ {idx_file.relative_to(wiki_dir)}: index.md should NOT contain YAML frontmatter (§6)"
                )
            else:
                result.valid_indexes += 1
        except Exception as e:
            result.errors.append(
                f"❌ {idx_file.relative_to(wiki_dir)}: Failed to read: {e}"
            )
    ...
    if log_file.exists():
        try:
            ...
            if not log_content.startswith("# "):
                result.errors.append("❌ log.md: Must start with a top-level H1 header")
                result.log_ok = False
            ...
            if not date_headers:
                result.warnings.append(
                    "⚠️  log.md: No YYYY-MM-DD date headers found. Ensure OKF date group format is followed."
                )
        except Exception as e:
...
def _print_report(
    ...
    print(
        f"✅ {result.valid_fm_count}/{concept_docs_count} documents have valid frontmatter"
    )
    print(
        f"✅ {result.non_empty_type_count}/{concept_docs_count} documents have non-empty 'type' field"
    )
...
def lint_wiki(wiki_dir_path: str) -> bool:
    ...
    all_md_files = walk_wiki_md_files(wiki_dir, include_raw_and_sources=False)
