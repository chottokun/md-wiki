...
def _initialize_wiki(wiki_dir: Path, qdrant_dir: Path):
    """WikiとQdrantのディレクトリを初期化する。"""
...
def _run_autonomous_expansion():
    """Lintを実行し、自律的な知識拡張を行う。"""
    print("[4/5] Running Lint for autonomous expansion...")
    run_cmd(["uv", "run", "python", "main.py", "--lint"])


def _check_page_quality(page_path: Path) -> tuple[list[str], list[str]]:
    """個別のWikiページの品質をチェックする。"""
    errors = []
    warnings = []
...
def _verify_wiki_quality(wiki_dir: Path) -> tuple[list[str], list[str]]:
    """Wiki全体の品質を検証する。"""
    ...
    if len(concepts) == 0:
        errors.append(
            "概念ページが生成されていません（Red-linkが抽出・処理されていません）"
        )
...
def test_full_rebuild():
    ...
    # 1. 初期化
    _initialize_wiki(wiki_dir, qdrant_dir)

    # 2. データ投入
    _ingest_test_data()

    # 3. Lint（自律拡張）
    _run_autonomous_expansion()

    # 4. 品質検証
    errors, warnings = _verify_wiki_quality(wiki_dir)
