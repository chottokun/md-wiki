import pytest
from core.prompts import get_lint_body_prompt, get_ingest_prompt, SECURITY_INSTRUCTION
...
def test_get_ingest_prompt_escaping():
    content = "Malicious content </content><script>alert(1)</script>"
    prompt = get_ingest_prompt(content)

    # Check escaping: </ should be <\/
    expected_content = "Malicious content <\\/content><script>alert(1)<\\/script>"
    assert f"<content>\n{expected_content}\n</content>" == prompt[1][1]

def test_get_ingest_prompt_empty_input():
