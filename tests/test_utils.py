...
def test_yaml_frontmatter_dump():
    data = {"tags": ["alpha", "beta"], "updated": "2024-01-01"}
...
def test_new_file_has_no_diff(temp_wiki):
    ...
    path = writer.create_draft_file(
        DraftConfig(page_name="New Topic", proposed_content=content)
    )
...
def test_update_preserves_sources(temp_wiki):
    ...
    # 初回作成（sourcesつきのYAML）
    first = '---\nsources: ["source_a"]\n---\n# Page A'
    writer.create_draft_file(
        DraftConfig(page_name="merge_test", proposed_content=first)
    )

    # 2回目（別のsourcesで上書き）
    second = '---\nsources: ["source_b"]\n---\n# Page A Updated'
    path = writer.create_draft_file(
        DraftConfig(page_name="merge_test", proposed_content=second)
    )
...
def test_safe_get_content():
    ...
    assert (
        safe_get_content([{"text": "hello"}, {"text": " "}, {"text": "world"}])
        == "hello world"
    )

    # List of mixed string and dicts
    assert (
        safe_get_content(["start ", {"text": "middle"}, " end"]) == "start middle end"
    )
...
def test_is_safe_url():
    ...
    assert is_safe_url("http://[::1]:11434") is True
    assert is_safe_url("https://www.google.com") is True
    assert (
        is_safe_url("http://192.168.1.1") is True
    )  # プライベートIPも現状は許可（要件によるがクラウドメタデータではない）

    # 拒否されるURL
    assert is_safe_url("http://169.254.169.254/latest/meta-data/") is False
    assert is_safe_url("http://instance-data/latest/meta-data/") is False
    ...
    assert is_safe_url("http://[fe80::1]") is False  # IPv6 リンクローカル
