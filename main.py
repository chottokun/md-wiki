...
os.environ.setdefault(
    "FASTEMBED_CACHE_PATH", str(Config.MODELS_CACHE_DIR / "fastembed")
)
...
def run_workflow(input_data: Dict[str, Any], auto_approve: bool = False):
    ...
    if current_state and "target_page" in current_state:
        ...
        print(
            f"Info: Check [[{current_state['target_page']}]] in Obsidian and edit/remove tags."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input",
        nargs="?",
        help="ファイルパス（インジェスト時）またはトピック名（クエリ/メンテナンス時）",
    )
    parser.add_argument(
        "--query", "-q", action="store_true", help="クエリ（質問回答）モードを実行"
    )
    parser.add_argument(
        "--maintenance",
        "-m",
        action="store_true",
        help="メンテナンス（景観要約）モードを実行",
    )
    parser.add_argument(
        "--lint",
        "-l",
        action="store_true",
        help="Wikiの健康診断（リンク切れ・風化チェック）を実行",
    )
    parser.add_argument(
        "--sync",
        "-s",
        action="store_true",
        help="Wikiファイルを正としてQdrantを再構築（同期）する",
    )
    parser.add_argument(
        "--refine",
        "-r",
        action="store_true",
        help="手動編集したWikiページをAIエディターで洗練させる",
    )
    parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
        help="確認プロンプトをスキップして自動承認する",
    )
    parser.add_argument(
        "--force", "-f", action="store_true", help="未審査タグを無視して同期する"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="ファイルへの書き込みを行わず、変更内容をログに出力する",
    )
    ...
    try:
        if args.query:
            ...
            if not args.input:
                print(
                    "洗練対象のWikiページ名（またはファイルパス）を入力してください。"
                )
            else:
                ...
                filename = (
                    args.input if args.input.endswith(".md") else f"{args.input}.md"
                )
                ...
                if not full_path.exists():
                    ...
                    if "<<<<<<<" in content:
                        print(
                            "🚨 衝突マーカーを検知しました。コンフリクト解決モードで開始します。"
                        )
                        run_workflow(
                            {
                        ...
                        if not diff:
                            logger.info(
                                "未コミットの差分が見つからないため、ファイル全体を対象に洗練を実行します。"
                            )
                            diff = content
