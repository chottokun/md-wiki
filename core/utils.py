...
def is_safe_url(url: str) -> bool:
    """
    SSRF対策として、URLが安全かどうかを検証する。
    許可されるのは http/https スキームで、かつ不当なIP（リンクローカル等）でないもの。
    """
    try:
        ...
        # localhost/loopbackは許可（ローカルLLM用途を想定）
        ...
        try:
            ...
            # リンクローカル(169.254.0.0/16)やマルチキャストなどを拒否
            if (
                ip.is_link_local
                or ip.is_multicast
                ...
                # is_loopbackは上の個別チェックで通しているはずだが念のため
                if hostname.lower() not in ("127.0.0.1", "::1"):
                    return False
                return True

            # RFC 1918 (プライベートネットワーク) は、組織内利用の可能性を考慮して
            # デフォルトでは許可するが、環境変数等で制御可能にするのが望ましい。
            # 今回はSSRF対策の主目的であるクラウドメタデータ(169.254.169.254)の遮断を優先。
